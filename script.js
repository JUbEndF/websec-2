            ...
            for (let j = 0; j < stops.length; j++) {
                if(stops[j].stop_id === KS_ID)
                {
                    innerElement.push(`<a href=stop.html?stopid=${stops[j].stop_id}&label=${stops[j].label}">${stops[j].label}</a>` + "  " +
                                        + Math.ceil(res[i].getElementsByTagName("time")[0].textContent/60) + " минут(ы)" + "<br/>");
                    break;
                }
            }
        }
...
async function transportStop(stopID, ListTransport) {
    ...
    for (let i = 0; i < res.length; i++)
    {
        innerElement += `<a href="stopsroutetransport.html?hullNo=${res[i].getElementsByTagName("hullNo")[0].childNodes[0].nodeValue}&name=${res[i].getElementsByTagName("number")[0].childNodes[0].nodeValue + "  " + res[i].getElementsByTagName("type")[0].childNodes[0].nodeValue}">` + 
            res[i].getElementsByTagName("number")[0].childNodes[0].nodeValue + "  " + res[i].getElementsByTagName("type")[0].childNodes[0].nodeValue
            + `</a>` + ' ' + res[i].getElementsByTagName("time")[0].childNodes[0].nodeValue + " Минут" + "<br/>";
